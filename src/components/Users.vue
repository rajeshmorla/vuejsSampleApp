...
<div id="country-list" class="data-container">
  ...
  <md-toolbar>
    ...
    <md-button class="md-icon-button" v-on:click.native="updateTableData">
  ...
  <md-table md-sort="name" md-sort-type="desc" @select="onSelect" @sort="onSort">
    ...
    <md-table-body>
      <md-table-row v-for="(row, rowIndex) in tableDataCurrentPage" :key="rowIndex" :md-item="row" md-auto-select md-selection>
        <md-table-cell v-for="(column, columnIndex) in columnList" v-html="row[column] != 'undefined' ? heiglightSearchMatch(row[column],column) : '--'" :key="columnIndex" :md-numeric="false">
          ag
        </md-table-cell>
      </md-table-row>
    </md-table-body>
